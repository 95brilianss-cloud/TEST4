...
// Version: 1.7.0 (Full Fix with Photo Validation)
...
const GAS_URL = "https://script.google.com/macros/s/AKfycbxfj6tAWILhZKz47_Ibn_0iYK_Bc4NEEP6xQXDZsE-kY8ZndgvAn-JM7RSUU6P-4k-2/exec";
...
// NEW: State untuk foto parameter logsheet
let currentParameterPhoto = null; // Foto untuk parameter turbine saat ini
let currentCTPhoto = null;        // Foto untuk parameter CT saat ini
...
// ============================================
// 11. LOGSHEET FUNCTIONS (TURBINE) with Photo
// ============================================
...
function renderMenu() {
    ...
    Object.entries(AREAS).forEach(([areaName, params]) => {
        ...
        const filled = Object.keys(areaData).filter(k => !k.endsWith('_photo')).length;
        ...
        const hasPhoto = params.some(paramName => areaData[paramName + '_photo']);
            ...
            <div class="area-item ${isCompleted ? 'completed' : ''} ${hasAbnormal ? 'has-warning' : ''}" onclick="openArea('${areaName}')">
                ...
                <div class="area-info">
                    ...
                    <div class="area-meta ${hasAbnormal ? 'warning' : ''}">
                        ${hasPhoto ? '📷 ' : ''}${hasAbnormal ? '⚠️ Ada parameter bermasalah • ' : ''}${filled} dari ${total} parameter
                    </div>
                </div>
...
function updateOverallProgress() {
    ...
    let completedAreas = 0;
    Object.entries(AREAS).forEach(([areaName, params]) => {
        const filled = currentInput[areaName] ? Object.keys(currentInput[areaName]).filter(k => !k.endsWith('_photo')).length : 0;
...
function renderProgressDots() {
    ...
    for (let i = 0; i < total; i++) {
        ...
        const hasPhoto = currentInput[activeArea]?.[fullLabel + '_photo'];
...
function showStep() {
    ...
    const savedPhoto = currentInput[activeArea]?.[fullLabel + '_photo'];
    if (savedPhoto) {
        currentParameterPhoto = savedPhoto;
        updateParameterPhotoPreview(savedPhoto);
    } else {
        resetParameterPhoto();
    }
...
// NEW: Handler foto untuk Parameter Turbine
function handleTurbinePhoto(event) {
    ...
    reader.onload = function(e) {
        currentParameterPhoto = e.target.result;
        updateParameterPhotoPreview(currentParameterPhoto);
        showCustomAlert('✓ Foto parameter berhasil diambil!', 'success');
        
        // Auto save photo to draft
        const fullLabel = AREAS[activeArea][activeIdx];
        if (!currentInput[activeArea]) currentInput[activeArea] = {};
        currentInput[activeArea][fullLabel + '_photo'] = currentParameterPhoto;
...
function updateParameterPhotoPreview(photoData) {
    const preview = document.getElementById('paramPhotoPreview');
    const section = document.getElementById('paramPhotoSection');
    
    if (preview) {
        preview.innerHTML = `<img src="${photoData}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 12px;" alt="Parameter Photo">`;
...
function resetParameterPhoto() {
    currentParameterPhoto = null;
    const preview = document.getElementById('paramPhotoPreview');
    const section = document.getElementById('paramPhotoSection');
    ...
    if (preview) {
        preview.innerHTML = `
            <div class="photo-placeholder" style="text-align: center; color: #64748b;">
                ...
                <div style="font-size: 0.875rem;">Ambil Foto Parameter</div>
                <div style="font-size: 0.75rem; margin-top: 4px; opacity: 0.7;">Validasi realtime di lokasi</div>
            </div>
        `;
        preview.style.border = '2px dashed rgba(148, 163, 184, 0.3)';
    }
    if (section) {
        section.classList.remove('has-photo');
    }
    
    const camera = document.getElementById('paramCamera');
...
async function sendToSheet() {
    ...
    Object.entries(currentInput).forEach(([areaName, params]) => {
        Object.entries(params).forEach(([paramName, value]) => {
            if (paramName.endsWith('_photo')) {
                const realParamName = paramName.replace('_photo', '');
                allPhotos.push({
                    parameter: realParamName,
                    photo: value,
                    area: areaName
                });
            } else {
    ...
    console.log('Sending Logsheet Data with Photos:', finalData);
...
// ============================================
// 14. CT LOGSHEET FUNCTIONS with Photo
// ============================================
...
function updateCTOverallProgress() {
    ...
    let completedAreas = 0;
    Object.entries(AREAS_CT).forEach(([areaName, params]) => {
        const filled = currentInputCT[areaName] ? Object.keys(currentInputCT[areaName]).filter(k => !k.endsWith('_photo')).length : 0;
        if (filled === params.length && filled > 0) completedAreas++;
    });
    updateCTOverallProgressUI(completedAreas, totalAreas);
}
...
function renderCTProgressDots() {
    ...
    for (let i = 0; i < total; i++) {
        ...
        const hasPhoto = currentInputCT[activeAreaCT]?.[fullLabel + '_photo'];
        const lines = savedValue.split('\n');
        const firstLine = lines[0];
...
// NEW: Handler foto untuk Parameter CT
function handleCTPhoto(event) {
    ...
    reader.onload = function(e) {
        ...
        updateCTPhotoPreview(currentCTPhoto);
        showCustomAlert('✓ Foto parameter CT berhasil diambil!', 'success');
        
        // Auto save photo to draft
        const fullLabel = AREAS_CT[activeAreaCT][activeIdxCT];
        if (!currentInputCT[activeAreaCT]) currentInputCT[activeAreaCT] = {};
        currentInputCT[activeAreaCT][fullLabel + '_photo'] = currentCTPhoto;
...
function updateCTPhotoPreview(photoData) {
    const preview = document.getElementById('ctParamPhotoPreview');
    const section = document.getElementById('ctParamPhotoSection');
    
    if (preview) {
        preview.innerHTML = `<img src="${photoData}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 12px;" alt="CT Photo">`;
        preview.style.border = '2px solid #10b981';
    }
    if (section) {
        section.classList.add('has-photo');
    ...
    if (preview) {
        preview.innerHTML = `
            <div class="photo-placeholder" style="text-align: center; color: #64748b;">
                ...
                <div style="font-size: 0.875rem;">Ambil Foto Parameter CT</div>
                <div style="font-size: 0.75rem; margin-top: 4px; opacity: 0.7;">Validasi realtime di lokasi</div>
            </div>
        `;
        preview.style.border = '2px dashed rgba(148, 163, 184, 0.3)';
...
function showCTStep() {
    ...
    // Load saved photo if exists
    const savedPhoto = currentInputCT[activeAreaCT]?.[fullLabel + '_photo'];
    if (savedPhoto) {
        currentCTPhoto = savedPhoto;
        updateCTPhotoPreview(savedPhoto);
    } else {
        resetCTPhoto();
    }
...
function setupParameterPhotoListeners() {
    // Setup listener untuk foto parameter Turbine
    const paramCamera = document.getElementById('paramCamera');
    if (paramCamera) {
        paramCamera.addEventListener('change', handleTurbinePhoto);
    }
    
    // Setup listener untuk foto parameter CT
    const ctParamCamera = document.getElementById('ctParamCamera');
    if (ctParamCamera) {
        ctParamCamera.addEventListener('change', handleCTPhoto);
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    setupLoginListeners();
    setupTPMListeners();
    setupParameterPhotoListeners(); // NEW: Setup photo listeners
    
    simulateLoading();
    
    console.log(`${APP_NAME} v${APP_VERSION} initialized successfully with Photo Validation`);
});
